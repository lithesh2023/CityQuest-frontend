...
async function proxy(req: NextRequest, locationId: string) {
  ...
  const target = `${base}/api/admin/locations/${encodeURIComponent(locationId)}`;
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ locationId: string }> }) {
  const { locationId } = await ctx.params;
  return proxy(req, locationId);
}
