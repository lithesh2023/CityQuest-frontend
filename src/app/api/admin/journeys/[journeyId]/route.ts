...
async function proxy(req: NextRequest, journeyId: string) {
  ...
  const target = `${base}/api/admin/journeys/${encodeURIComponent(journeyId)}`;
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ journeyId: string }> }) {
  const { journeyId } = await ctx.params;
  return proxy(req, journeyId);
}
